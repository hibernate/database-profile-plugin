...
class DatabaseAllocator {
    ...
    public static DatabaseAllocator locate(Project project) {
        if ( ! project.getRootProject().hasProperty( DB_ALLOCATOR_KEY ) ) {
            project.getRootProject().getExtensions().getExtraProperties().set(
                    DB_ALLOCATOR_KEY,
                    new DatabaseAllocator( project.getRootProject() )
            );
        }
    ...
    public DatabaseAllocation getAllocation(Profile profile) {
		DatabaseAllocation databaseAllocation = databaseAllocationMap.get( profile.getName() );
		if ( databaseAllocation == null ) {
			databaseAllocation = createAllocation( profile );
			databaseAllocationMap.put( profile.getName(), databaseAllocation );
		}
